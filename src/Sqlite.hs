...
instance (RowDDL Sqlite a, KnownSymbol n, NamesList pk)
  ...
  where
    createTable _ (Proxy :: Proxy (Table n a pk)) = runSqliteDDL
    ...
    dropTable _ (Proxy :: Proxy (Table n a pk)) = runSqliteDDL
        $ format "DROP TABLE {}" $ Only $ symbolVal' (proxy# :: Proxy# n)

runSqliteDDL :: (MonadIO m) => TL.Text -> DbMonad Sqlite m ()
runSqliteDDL cmd = ask >>= \conn -> liftIO (exec conn $ TL.toStrict cmd)

instance (NamesList a, KnownSymbol t, RowDDL Sqlite a)
    => Ins Sqlite (Table t a pk)
  where
    ins _ (Table rec :: Table t a pk) = insSqlite (proxy# :: Proxy# t) rec

instance AutoGenPK Sqlite Int64 where
    getPK _ = ask >>= \conn -> liftIO (lastInsertRowId conn)

instance (KnownSymbol t, NamesList (Diff a pk)
        , AutoGenPK Sqlite pk, RowDDL Sqlite (Diff a pk))
    => InsAutoPK Sqlite (Table t a pk)
  where
    insAuto _ _ (rec :: DataRow (Table t a pk))
        = insSqlite (proxy# :: Proxy# t) rec >> getPK sqlite

insSqlite :: (KnownSymbol t, RowDDL Sqlite r, NamesList r, MonadIO m)
    => Proxy# (t::Symbol) -> r -> DbMonad Sqlite m ()
